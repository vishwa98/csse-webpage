<template>
  <v-container id="regular-tables" fluid tag="section">
    <v-btn to="/tables/addDriver">Add Driver</v-btn>
...
export default {
  ...
  methods: {
    ...
    ...mapActions(["setDriversAction"]),

    close() {
      this.dialog = false;
      setTimeout(() => {
        this.driverData = Object.assign({}, this.default);
        this.editedIndex = -1;
      }, 3000000);
    },
    ...
    loadDrivers: async function() {
      ...
      let apiURL = "http://localhost:8000/api/viewDrivers";
      axios
        .get(apiURL)
        .then((res) => {
          this.posts = res.data;
    ...
    editDriver(item) {
      ...
      this.driverData = Object.assign({}, item);
      this.dialog = true;
    },

    updateDriver() {
      if (this.$refs.driverData.validate() && this.driverData.route != "") {
        let apiURL = `http://localhost:8000/api/updateDrivers/${this.driverData._id}`;
        axios
          .post(apiURL, this.driverData)
          .then((res) => {
            console.log(res);
            this.close();
            this.loadDrivers();
            this.color = "info";
            this.text = "Driver has been updated";
            this.snackbar = true;
          })
          .catch((error) => {
            console.log(error);
    ...
    deleteDriver(id) {
      this.$swal({
        ...
        if (result.value) {
          try {
            let apiURL = `http://localhost:8000/api/deleteDriver/${id}`;
            ...
            this.$swal("Deleted!", "Driver has been deleted", "success");
            this.color = "error";
            this.text = "Driver has been deleted";
            this.snackbar = true;
          } catch (error) {
